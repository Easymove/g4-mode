...
(defvar g4-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "C-M-h u") 'g4-highlight-unused)
    (define-key map (kbd "C-M-h d") 'g4-highlight-undefined)
    (define-key map (kbd "C-M-h z") 'g4-unhighlight-all)
    (define-key map (kbd "C-M-t a") 'g4-add-terminals)
    (define-key map (kbd "C-M-t d") 'g4-delete-terminals)
    (define-key map (kbd "C-M-t c") 'g4-clean-terminals)
    (define-key map (kbd "C-c C-l") 'parse-g4)
    (define-key map (kbd "C-d d") 'g4-dump-to-dot)
    (define-key map (kbd "C-F") 'g4-first)
    (define-key map (kbd "C-M-F") 'g4-follow)
    map))
