              ...
              (lambda (x)
                (when (typep x 'rule-node)
                  (let ((first-list))
                    (traverse x
                              (lambda (y)
                                (when (typep y 'command-node)
                                  (push (command-first-set y) first-list))))
                    (mapc (lambda (set1)
                            (mapc (lambda (set2)
                                    (let* ((first1 (mapcar #'node-string-name set1))
                                           (first2 (mapcar #'node-string-name set2))
                                           (inters (intersection first1 first2 :test #'equal)))
                                      (when (and (not (equal first1 first2))
                                                 inters)
                                        (push (node-string-name x) conflicts)
                                        (setf res nil))))
                                  first-list))
                          first-list)))))
    (if res
